    ...
    <div className={fullscreen
      ...
      <div className="flex justify-end mb-2">
        <button
          ...
          {fullscreen ? (
            <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 9V4.5M9 9H4.5M15 9h4.5M15 9V4.5M9 15v4.5M9 15H4.5M15 15h4.5M15 15v4.5" /></svg> exit fullscreen</>
          ) : (
            <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5v-4m0 4h-4m4 0l-5-5" /></svg> fullscreen</>
          )}
        </button>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-12 items-stretch">
        ...
        {/* LEFT — sticky demo */}
        {(link || image) && (
          <div className="w-full lg:w-[58%] lg:sticky lg:top-10 flex flex-col">
        ...
        <div className="w-full lg:w-[42%] flex flex-col gap-8">
          ...
          {problem && (
            <div>
              ...
              <p className="text-neutral-200 text-sm leading-relaxed">{problem}</p>
            </div>
          )}

          {solution && (
            <div>
              <p className="text-xs uppercase tracking-widest text-neutral-400 mb-2">The Solution</p>
              <p className="text-neutral-200 text-sm leading-relaxed">{solution}</p>
            </div>
          )}

          {diagram && (
            <div>
              <p className="text-xs uppercase tracking-widest text-neutral-400 mb-2">Architecture / Flow</p>
          ...
          {impact && (
            <div>
              <p className="text-xs uppercase tracking-widest text-neutral-400 mb-2">Impact</p>
          ...
          {techStack?.length > 0 && (
            <div>
              <p className="text-xs uppercase tracking-widest text-neutral-400 mb-2">Tools</p>
          ...
          {description && (
            <div className="border border-neutral-700 rounded-xl overflow-hidden">
              <button
                ...
                <span className="text-neutral-500 text-base leading-none">{descOpen ? "−" : "+"}</span>
              </button>
              {descOpen && (
